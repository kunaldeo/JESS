...
void on_reprise(void) {
    ...
    if (lys.reprise == OUI) {
        ...
        } else /* il y a eu un flash y a pas longtemps, donc on fait juste des etoiles */
        {
            ...
            if ((conteur.freeze_mode == 0) && (conteur.mix_reprise > 5) &&
                (conteur.draw_mode != 2)) /* si c pas freeze_mode on change tout */
            {
                conteur.blur_mode = rand() % 5;
            }
        }
...
void render_deformation(int defmode) {
    ...
    if (video == 8) {
        printf("This block video 8 got executed\n");
        buf = buffer;
        tab1 = table1;
        tab2 = table2;
        tab3 = table3;
        ...
        for (i = 0; i < resy * resx; i++) {
            aux = (unsigned char *) ((*(tab1) << 2) + buffer);
            *pix = *(aux);
            pix++;
            *pix = *(aux + 1);
            pix++;
...
void render_blur(int blur) {
    ...
    if (video == 8) {
        ...
        pitch_4 = pitch + 4;
        bmax = pitch * (resy - 1) + pixel;
        for (pix = pixel; pix <  bmax - 4;) {
